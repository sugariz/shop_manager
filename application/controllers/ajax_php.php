<?php
    class ajax_php extends CI_Controller{
        ...
        public function load_table_sale_details(){
            $q = intval($_GET['q']);
            $stt = 0;
            $arr = $this->Order_model->getListAt($q);
            $html="";
            foreach($arr as $key=>$value){
                $stt++;
                $html.= "<tr>
            ...
            $html.="<tr>
            <td colspan='2'><strong>Total</strong></td>
            <td>".$this->Sale_model->getSaleListAt($q)['soluong']."</td>
            <td></td>
            <td>".$this->Sale_model->getSaleListAt($q)['tongcong']."</td>
            </tr>
            <tr>";
            echo $html;
        }
